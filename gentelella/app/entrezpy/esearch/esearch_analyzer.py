...
from app.entrezpy.esearch import esearch_result
...
class EsearchAnalyzer(analyzer.EutilsAnalyzer):
  ...
  :class:`esearch_result.EsearchResult` instance.
  """
  def __init__(self):
    """:ivar result: :class:`esearch_result.EsearchResult`"""
    super().__init__()
    self.result = None

  def init_result(self, response, request):
    """Inits :class:`esearch_result.EsearchResult`.

    :return: if result is initiated
    :rtype: bool
    """
    if not self.result:
      self.result = esearch_result.EsearchResult(response, request)
      return True
    return False

  def analyze_result(self, response, request):
    """Implements :meth:`analyzer.EsearchAnalyzer.analyze_result`.
  ...
  def analyze_error(self, response, request):
    """Implements :meth:`analyzer.EutilsAnalyzer.analyze_error`.
