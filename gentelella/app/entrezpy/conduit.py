...
from app.entrezpy.esearch import esearcher
from app.entrezpy.esearch import esearch_analyzer
from app.entrezpy.elink import elinker
from app.entrezpy.elink import elink_analyzer
from app.entrezpy.epost import eposter
from app.entrezpy.epost import epost_analyzer
from app.entrezpy.efetch import efetcher
from app.entrezpy.esummary import esummarizer
...
class Conduit:
  ...
  def search(self, query, analyzer=esearch_analyzer.EsearchAnalyzer):
    ...
    :return: analyzer
    :rtype: :class:`esearch_analyzer.EsearchAnalyzer`
    """
    analyzer = query.analyzer if query.analyzer else analyzer()
    return esearcher.Esearcher(self.tool,
                                                self.email,
                                                self.apikey,
                                                threads=self.threads,
                                                qid=query.id).inquire(query.parameter, analyzer)

  def summarize(self, query, analyzer=esummary_analyzer.EsummaryAnalyzer):
    ...
    :param analyzer: reference to analyzer class
    :return: analyzer
    :rtype: :class:`esummary_analyzer.EsummaryAnalyzer`
    """
    analyzer = query.analyzer if query.analyzer else analyzer()
    return esummarizer.Esummarizer(self.tool,
                                                     self.email,
                                                     self.apikey,
                                                     threads=self.threads,
                                                     qid=query.id).inquire(query.parameter, analyzer)
  ...
  def link(self, query, analyzer=elink_analyzer.ElinkAnalyzer):
    ...
    :param analyzer: reference to analyzer class
    :return: analyzer
    :rtype: :class:`elink_analyzer.ElinkAnalyzer`
    """
    analyzer = query.analyzer if query.analyzer else analyzer()
    return elinker.Elinker(self.tool,
                                          self.email,
                                          self.apikey,
                                          threads=self.threads,
                                          qid=query.id).inquire(query.parameter, analyzer)
  ...
  def post(self, query, analyzer=epost_analyzer.EpostAnalyzer):
    ...
    :param analyzer: reference to analyzer class
    :return: analyzer
    :rtype: :class:`epost_analyzer.EpostAnalyzer`
    """
    analyzer = query.analyzer if query.analyzer else analyzer()
    return eposter.Eposter(self.tool,
                                          self.email,
                                          self.apikey,
                                          threads=self.threads,
                                          qid=query.id).inquire(query.parameter, analyzer)

  def fetch(self, query, analyzer=efetch_analyzer.EfetchAnalyzer):
    ...
    :return: analyzer
    :return: analyzer
    :rtype: :class:`efetch_analyzer.EfetchAnalyzer`
    """
    analyzer = query.analyzer if query.analyzer else analyzer()
    return efetcher.Efetcher(self.tool,
                                             self.email,
                                             self.apikey,
                                             threads=self.threads,
                                             qid=query.id).inquire(query.parameter, analyzer)
