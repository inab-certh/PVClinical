		...
		constructor(params) {
			...
			this.disableExportAnalysis = ko.pureComputed(() => {
				return this.dirtyFlag().isDirty() || !this.sources().some(si => si.info() && si.info().executionInfo.status === constants.status.COMPLETE);
                        ...
                        self=this;
                        this.disableExportAnalysis.subscribe(function() {
                            if(!self.disableExportAnalysis()) {
                                var selSource = self.selectedSource()? self.selectedSource():self.sources()[0];
                                self.selectSource(selSource);
                            }
                        });
