...
#   print( quartervals[startloc] )
  ...
  if ( myevent == '')
  {
    ...
    curinds <-  which( prrtab$e==myevent )
    prrtab <- prrtab[ curinds ,]
  }
  if (startloc < length( quartervals ) & ( myevent != '') )
  {
    ...
    curinds <-  which( detable$e==myevent )
...
#    browser()
    ...
    {
    k <- match( prrtab[i, 'd'],  starttable$d )
    if( !is.na(k) )
      {
...
# Calculations ========= #Calculate PRR and put in merged table
  getprr <- reactive({
    ...
    fulltable <- loadquarter()$fulltable
    if ( is.null( detable ) )
      {
...
#    browser()
    ...
    for ( i in 1:nrow(mydf) )
      {
        curinds <- which( fulltable$d == mydf$d[i] )
        mysum <- sum( fulltable[curinds, 'A'])
        mydf[i, 'AB'] <- mysum
      }
    } 
    # A = drug-event count
...
#                            PRR= round(mydf$PRR, digits=2), LB=round(mydf$lb, digits=2), UB=round(mydf$ub, digits=2), row.names=NULL)
    sourcedf <- data.frame(Event=mydf$d, drugcounts= as.integer(mydf$A), 
                           allcounts=mydf$AB, row.names=NULL)

    comb <- sourcedf
    eventcounts <- unique( comb[, c('Event', 'drugcounts', 'allcounts')] )
    totals <- list( total=loadquarter()[['totalevents']], 
                    totaldrug=unique(mydf$AC) )
...
#      mystats <- calcLRTstats(totals, eventcounts, allevents=n..)
      mystats <- calcLRTstats2( nij=nij, n.j=n.j, ni.=ni., n..= n.. )
