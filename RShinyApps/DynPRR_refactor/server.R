...
getexactdrugvar <- function(){
  return ( paste0(getdrugvar()) )
}

getexactaevar <- function(){
  return ( paste0(getaevar())  )
}
...
getquery_de <- reactive({
  ...
  # con <- mongo("fda", url = "mongodb://sdimitsaki:hXN8ERdZE6yt@83.212.101.89:37777/FDAforPVClinical?authSource=admin")
  
  drugName<-q$t1
  eventName<-q$t2
  
  drugeventQuery<-createDateDrugEventQuery(drugName=drugName, eventName=eventName)
  drugeventResult <- con$aggregate(drugeventQuery)
  colnames(drugeventResult)[1]<-"time"
  drugeventResult$time <- as.Date(drugeventResult$time, tz = "HST")
  mylist<-drugeventResult
  # Redone
...
getquery_d <- reactive({
  ...
  drugQuery<-createDateDrugQuery(drugName=drugName)
  drugResult <- con$aggregate(drugQuery)
  colnames(drugResult)[1]<-"time"
  drugResult$time <- as.Date(drugResult$time, tz = "HST")
  mylist<-drugResult
  # Redone
  return( list( mydf=mylist, exact = q$t1) )
}) 
...
getquery_e <- reactive({
  ...
  eventQuery<-createDateEventQuery(eventName=eventName)
  eventResult <- con$aggregate(eventQuery)
  colnames(eventResult)[1]<-"time"
  eventResult$time <- as.Date(eventResult$time, tz = "HST")
  mylist<-eventResult
  # Redone
  return( list( mydf=mylist, exact = q$t2) )
})    
...
getquery_all <- reactive({
  ...
  allQuery<-createDateAllQuery()
  allResult <- con$aggregate(allQuery)
  colnames(allResult)[1]<-"time"
  allResult$time <- as.Date(allResult$time, tz = "HST")
  tmp<-allResult
  # Redone
...
getcoeventlist <- reactive({
  ...
  drugQuery<-createDrugEventQuery(drugName=drugName)
  drugResult <- con$aggregate(drugQuery)
  colnames(drugResult)[1]<-"term"
