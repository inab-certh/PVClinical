...
# 
# sexResult<-con$aggregate(createSexQuery('OMEPRAZOLE'))
# 
# result<-con$find(createQuery(drugName ='OMEPRAZOLE',startDate='2019-01-01',endDate = '2020-01-01' ))
                      ...
                      seriousnesslifethreatening=NULL,seriousnessother=NULL)
{
  ...
  if (!is.null(drugName)){
    if (!noConc)
      ...
      fda_query$drug <- paste0("1_",toupper(drugName))
  }
...
createIDquery<-function(id){
  paste0('[{ "$match" :
              { "safetyreportid": {
                  "$in" : ["',paste(id,collapse='","'),'"]
              }
              }
              },
         { "$project":
         {"patient.drug": 1, "receiptdate" : 1}
           }
           ]')
