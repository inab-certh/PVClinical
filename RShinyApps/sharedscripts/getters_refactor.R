...
getcounts999 <- function( session, v, t, count, limit=1000, 
                         exactrad='exact', counter=1, db= '/drug/',eventName=NULL, drugName = NULL )
  {
  if ( is.null( t ) ){
...
#   mydf <- mylist$result
  ...
  if ( v[2] == "patient.reaction.reactionmeddrapt.exact"){
    ...
    eventQuery<-createEventallDrugQuery(eventName=eventName)
...
#Refactor
# getcounts999 <- function( t, startDate=NULL, endDate=NULL, eventName=NULL ){
#   query <- createQuery(drugName=t,eventName=eventName,startDate=startDate,endDate=endDate)
#   mydf <- eventTable(query)
#   
#   excludeddf <- data.frame()
#   if( length(mydf)>0 )
#   {
...
#   
#   
#   return( list(mydf=mydf, excludeddf = excludeddf   ) )
# }
#Redone
